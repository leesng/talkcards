...
- `./build.sh` (repo root) — cross-compiles `target/talkcards-linux-{amd64,arm64}` and `target/talkcards-windows-amd64.exe` single binaries (git-ignored). `./build.sh test` builds the native-arch binary, boots it on a temp port (`TEST_PORT`, default 8765), and runs the full-game audit `e2e-audit.js` against it.
...
- 人机模式（服务器侧机器人，`internal/bot` 策略：首出 ≤8 小牌（单/双/三不限型）按牌面升序最优先、小牌清完后按 孤张单→对→三→炸弹 组序、不拆组、炸弹先比张数再比牌面；跟牌走 `FollowSmart` 智能策略：①队友的牌除非是 ≤8 的单/双/三一律不吃，只吃对手的牌；②无整组常规牌能压时可拆 A/2/大小王的恰 2 张对子跟对手的单牌（拆一张单跟，另一张留手；三条/炸弹不拆）；③桌面无分且只剩炸弹能压时 ≤4×8 炸弹 20%、4 张大牌面 10%、超过 4 张（含王炸）5% 放行；④桌面有分且 ≤50 时 4 张炸弹 80%、5 张 40%、6 张及以上含王炸 10%，分 >50 必压，随机源可注入便于测试，概率分档常量集中在 `internal/bot/static.go`）：主持位 `HOST_START_GAME {fillBots:true}` 填充空位为机器人（不传 fillBots 且有空位 → MESSAGE 提示拒绝）；机器人无真实连接，`hub.scheduleBotIfTurn` 每桌单定时器、随机延迟后持锁重验轮次再走与真人共用的 `applyPlayResult`（origin=nil 不回 SUCCESS/ERROR）；终局/终止时 `clearBotSeats` 清座广播。前端以 `isBot` 标记展示。回归测试 `internal/hub/bots_test.go`：8 机器人自打整局 + 单真人 e2e 策略整局（注意：fakeConn 帧经 pump 异步投递，出牌后必须轮询等待 SUCCESS/ERROR 帧，同步读会误判被拒→重出同一张→越序毒化）。
...
- `internal/game/` — 牌局状态机（对齐 game-rules.md 的有意偏离版）：`Phase`(Idle/Playing/Over/Error，无叫分阶段——开局先手直接首出)、规则链 `playRules`、`Play(pos,cards) PlayResult{Accepted,Applied,HasShape,Shape}`（越序合法牌 → Accepted+!Applied+PhaseError）；快照 getter `SumFeng()` 返回 **[8]int 数组**、`Hands()` 返回 `[]HandSnapshot{PosID,Cards}`（wire 数字键对象、红桃统计 `HT`、`Result` 的 JSON 字段名均由 hub translator/events 承载，game 包无 json tag）；`settle.go TeamScores()` 供落库。
...
- 断线重连：游戏进行中（出牌阶段）掉线保留座位（座位/手牌/轮转原样），`--reconnect-timeout`（默认 600s，0=无限）内重登自动坐回（LOGIN_SUCCESS 后紧跟 RECONNECT + 按阶段重放 GAME_START[当前手牌]/SHOW_TOP_CARD+CTX_PLAY_CHANGE[缓存帧，首出前掉线则补发轮转帧——否则重连者不知轮到自己会死锁对局]）；超时未归不再直接判逃终止，而是**转自动托管续局**（保留记录仍可随时重连接管）；仅当全部真实玩家都断线超时（无任何在线真人）才直接按逃跑终止并落库。非游戏中掉线/主动退出仍即时释放座位。
- 观战模式（2026-09）：大厅点击**进行中对局的任意座位**即观战（无独立按钮）。Session 以 `deskID=D, posID=-1` 表示观战者，`broadCastRoom` 自动命中所有房间帧；唯一脱敏点是 GAME_START——`broadCastGameStart` 给玩家发全量手牌、给观战者发 `redactGameStart`（每手 cards 空 + `count` 张数，张数可见牌面不可见）；中途加入经 `replayGameFrames(c,d,redacted)`（与 resumeClient 共用的重放逻辑）恢复桌面/轮转/sumFeng。事件：`SPECTATE{deskId}` → `SPECTATE_SUCCESS{deskId,posInfos,hostPosId}` / `SPECTATE_ERROR{msg}`（非进行中拒绝）；**入座玩家（含对局中）一律不能观战**（MESSAGE 拒绝，对局中座位不可释放转观战）；观战者 PLAY_CARD/PREPARE/SITDOWN/USER_MESSAGE 等经 `seatedClient` 守卫拒绝（UNSITDOWN=退出观战、直接回大厅不可反向准备，断线只清会话不影响对局）；观战进出广播 SYS `USER_MESSAGE{posId:-1}`（前端 posId<0 显示"观战"）。开局/终局经 `STATUS_CHANGE{posId:-1,deskState:2|0}`（`DeskState *int`，0 不被 omitempty 吞掉）维护大厅桌状态，前端据此把座位点击路由为观战；大厅进行中桌角标只显示「观战」二字（`.desk-live`，位于中央桌号圆下方空档、`pointer-events:none` 不挡座位点击）。前端以 posId=3 视角渲染（`isSpectator` 隐藏操作/聊天、手牌区显示牌背占位、CTX 帧按 `ctxData.len` 递减张数、GAME_OVER 走简化弹窗、autoPlayCards 等全部守卫）。回归：`internal/hub/spectate_test.go` + `backend/scripts/spectate-smoke.js`（真 WS 冒烟，注意 attachDriver 要在 HOST_START_GAME 前挂、budget 用 `moveBudget(n)`）。
- 托管（trustee）：`TOGGLE_TRUSTEE`（无载荷，仅对局进行中生效，机器人座位不可托管）→ 广播 `TRUSTEE_CHANGE{posId,trustee}`；托管座位（`Seat.Trustee`）与机器人共用 `botTurn`/`botAct` 代打链路，出牌规则与机器人完全一致（internal/bot 策略）；托管中手动 PLAY_CARD 被拒（MESSAGE，防与代打竞态越序毒化）；重连/终局/终止自动清除托管标记（`clearTrustees` 广播复位）。前端：出牌阶段显示"托管/取消托管"按钮 + 座位"托管"角标，托管中隐藏出牌按钮且倒计时到点不自动出牌。回归测试 `internal/hub/trustee_test.go`。
...
- Bug-for-bug compat is intentional in the Go port: GAME_START broadcasts all 8 hands, escape flow setting empty seats' state — don't "fix" these without comparing against 原 Node 版 server.js（已删，查 git 历史）。原 `updateRoomStatus(desk,posID)` 传参怪癖已于 P3 修正为传 0（已查证前端只消费 deskId/positions[].posId/state/userName，不读 desk.state）。另一处有意收紧：原 Validate 无阶段门（原叫分阶段出 4+ 张炸弹可"压空桌"并把对局转入 Playing、空 PLAY_CARD 等效叫分），Go 版规则链含 rulePhasePlaying，非出牌阶段的 PLAY_CARD 一律拒绝且无状态变更。叫分流程本身已于 2026-09 整体移除（先手直接首出）。
- 规则已**有意偏离** Node 原版（2026-09 产品决策，见 game-rules.md），比对 git 历史时注意三处：① whoFirst 改为红桃3最多→比3总数→随机（原为红桃字典序含王）；② 王炸大小改为张数优先、同张数大王>小王，普通炸弹反压王炸需张数>2N-1（原 2N-1 阈值下大小王等价且允许 3王压4王、4炸压3王）；③ 全队出完终局带走败方未出手分牌，Result.Score 语义为"胜队最终得分"（前端 GAME_OVER 以"本局分差"一行展示）。e2e-audit.js 的 Replayer.beats 已同步压牌规则，改 game.Validate 必须同步改它。
