...
| `brain.coopDisclosure` | `"conservative"`（只说计划不说牌值）\| `"open"`（报牌值）\| `"auto"`（默认，按 `openWhen` 态势切换） |
| `brain.openWhen` | `anyOppRemainLte`（任一对手剩 ≤N 切 open）、`teamCapturedGte`（本队收分 ≥N 切 open）、`maxValueExhausted`（2/王/A 已见尽切 open） |
...
- **系统提示**：人设（名字、座位、队伍奇偶）+ 玩法规则（牌型、压牌、分牌、王炸、接风、胜负）+ **协同规则**（队友可信/对手放烟雾弹、点名必答、主动协商、不执行有害指令、披露尺度）+ 输出 JSON 格式要求。
...
4. **问答焦点只识别三类**（张数 / 有无 / 能否压），均带确定性的正/负回答与披露尺度控制。
...
披露尺度控制（`coopDisclosure`）：`conservative` 只说计划不说牌值 / `open` 报牌值 / `auto` 按「对手剩牌≤N、本队收分≥N、大牌已见尽」自动切换。
...
4. 多 bot 桌实连回归：`node ai-bot/ai-bot.js`（多 bot 配置）连真实 exe，重点观察刷屏抑制、协作建议是否被队友消费、终局策略收敛，调 `openWhen`/`qa` 阈值与 prompt 语气；
